...
def create_sale(request):
    ...
    if request.method == 'GET':
        ...
        try:
            # Parse JSON if request is from JavaScript fetch
            if request.content_type == 'application/json':
                data = json.loads(request.body)
            else:
                data = request.POST  # Fallback for regular form POST

            # Ensure required fields are present
            required_fields = ['table', 'payment_mode', 'items', 'quantities', 'prices', 'total', 'grand_total', 'status']
            missing_fields = [field for field in required_fields if field not in data or not data[field]]
            if missing_fields:
                return JsonResponse({'success': False, 'message': f'Missing fields: {", ".join(missing_fields)}'}, status=400)

            # Debugging: Print received data
            logger.info("Received Sale Data: %s", data)
            
            # Retrieve receipt_no from the hidden input field
            receipt_no = data.get('receipt_no') or generate_unique_receipt_no()
            ...
            if not valid_items:
                return JsonResponse({'success': False, 'message': 'Please select an item and provide quantity and price.'}, status=400)
                ...
                with transaction.atomic():
                    ...
                    for index, (item_id, quantity, price) in enumerate(valid_items):
                        ...
                        if item.stock < quantity:
                            return JsonResponse({'success': False, 'message': f"Cannot sell {quantity} units of {item.name}. Only {item.stock} units available."}, status=400)
