...
class Sale(models.Model):
    ...
    def qr_code_url(self):
        if self.qr_code:
            return os.path.join(settings.MEDIA_URL, self.qr_code.name)
        return None

    def save(self, *args, **kwargs):
        if not self.business:
            self.business = (
                getattr(self.user, 'admin_business', None)
                or self.user.owned_businesses.first()
                or self.user.businesses.first()
            )
            if not self.business:
                raise ValidationError("The user is not associated with any business.")
