...
from .models import Sale, Item, DailyReceipt, Expenditure, Table, Business, SubscriptionPlan, PriceHistory, StockEntry, Report, Category, UserProfile
...
from .serializers import (
    SubscriptionPlanSerializer, BusinessSerializer, ItemSerializer, PriceHistorySerializer, TableSerializer,SaleSerializer,
    StockEntrySerializer, ReportSerializer, DailyReceiptSerializer, CategorySerializer,ExpenditureSerializer, UserProfileSerializer
)
...
def generate_unique_receipt_no():
    ...
    logger.debug("Generating receipt number...")  # Add logging
    ...
    logger.error("Failed to generate a unique receipt number after multiple attempts.")
...
def dashboard(request):
    ...
    # Check if the user belongs to the "Client Admin" group
    if not request.user.groups.filter(name="Client Admin").exists():
        messages.warning(
            request, 
            "Access denied. Only Client Admin accounts can access this page. Contact admin if you need additional permissions."
        )
        return redirect('create-sale')
    ...
    # Fetch chart data via dashboard_data
    data = dashboard_data(request)
    if data.status_code != 200:
        messages.error(request, "Unable to load dashboard data.")
        return redirect('business-list')

    # Access the JSON data directly
    json_data = data.content  # Extract the raw JSON content
    json_data = json.loads(json_data)  # Parse the JSON content into a Python dictionary

    # Context for rendering with JSON-encoded lists and strings
    context = {
        'business': business,
        'sales_labels': json_data.get('sales_labels', []),
        'sales_data': json_data.get('sales_data', []),
        'stock_labels': json_data.get('stock_labels', []),
        'stock_data': json_data.get('stock_data', []),
        'predictions_labels': json_data.get('predictions_labels', []),
        'predictions_data': json_data.get('predictions_data', []),
        'api_dashboard_data_url': reverse('dashboard-data'),
    }
...
def create_sale(request):
    ...
    if not request.user or not request.user.is_authenticated:
        logger.error("User authentication failed. request.user is None or unauthenticated.")
        return JsonResponse({'success': False, 'message': 'User authentication failed.'}, status=403)
    ...
    if not selected_business or not user_has_business_access(request.user, selected_business):
        ...
        if user_businesses.exists():
            ...
            return JsonResponse({'success': False, 'message': 'No active businesses found'}, status=400)
    ...
    if request.method == 'GET':
        ...
        return render(request, 'sales/create_sale.html', {'items': items_with_images,'tables': tables})       
        ...
        required_fields = ['table', 'payment_mode', 'items', 'quantities', 'unit_selling_prices', 'total_amount', 'grand_total', 'status']
        missing_fields = [field for field in required_fields if field not in data or not data[field]]
        if missing_fields:
            logger.warning(f"Missing fields: {missing_fields}")  # ✅ Debugging missing fields
        ...
        unit_selling_prices = data.get('unit_selling_prices', [])
        if not items or not quantities or not unit_selling_prices:
            logger.error("Items, quantities, or prices arrays are empty.")
            return JsonResponse({'success': False, 'message': 'Items, quantities, and prices must not be empty.'}, status=400)

        if len(items) != len(quantities) or len(items) != len(unit_selling_prices):
            logger.error("Mismatch in lengths of items, quantities, and unit selling prices.")
            return JsonResponse({'success': False, 'message': 'Mismatch in array lengths.'}, status=400)

        # Retrieve receipt_no from the hidden input field
        receipt_no = data.get('receipt_no')
        if not receipt_no:
            return JsonResponse({'success': False, 'message': 'Receipt number is required.'}, status=400)
        
        tip = float(data.get('tip', 0.00))                
        ...
        items_dict = {item.id: item for item in Item.objects.filter(business=selected_business, id__in=items)}
        
        # Prepare valid items
        valid_items = []
        for item_id, quantity, price in zip(items, quantities, unit_selling_prices):
            item = items_dict.get(item_id, business=selected_business)
            if not item:
                logger.error(f"Item with ID {item_id} not found.")
                return JsonResponse({'success': False, 'message': f"Item with ID {item_id} does not exist."}, status=400)

            if int(quantity) <= 0 or float(price) <= 0 or item.stock < int(quantity):
                logger.error(f"Invalid data for item {item_id}.")
                return JsonResponse({'success': False, 'message': f"Invalid data for item {item.name}."}, status=400)

            valid_items.append((item, int(quantity), float(price)))
                
        with transaction.atomic():
            total_amount = 0
            sales_records = []

            for index, (item, quantity, price) in enumerate(valid_items):
                ...
                sale = Sale(
                    ...
                    business=selected_business,
                    tip=tip if index == 0 else 0,  # Assign tip to the first item
                )
            ...
            logger.info(f"QR Code Path: {qr_code_file_path}")
            ...
            logger.info(f"Final API Response: receipt_no={receipt_no}, id={sale.id}, total_amount={sale.total_amount}, status={sale.status}")
        ...
        return JsonResponse({
            ...
            'sales': [{'item_id': sale.item.id, 'item_name': sale.item.name, 'quantity': sale.quantity, 'total_amount': sale.total_amount} for sale in sales_records],
            'tip': sale.tip,  # ✅ Include tip amount
            'grand_total': sale.total_amount + sale.tip,  # ✅ Include grand total
            'status': sale.status,
            'customer_name': sale.customer_name,  # ✅ Include customer name
            'customer_number': sale.customer_number,  # ✅ Include customer contact
            'payment_mode': sale.payment_mode,  # ✅ Include payment mode
            'served_by': sale.user.get_full_name() or sale.user.username,  # ✅ Include served_by field
            'qr_code': qr_code_file_path  # ✅ QR Code for the receipt
        }, status=201)

        logger.debug(f"Backend Response Data: {response_data}")
        return JsonResponse(response_data, status=201)
    
    logger.debug(f"Received sale data: {json.dumps(data, indent=2)}")
...
def manage_users(request):
    # Ensure only client admins can access this view
    if not request.user.groups.filter(name="Client Admin").exists():
        messages.error(request, "Access Denied: Only client admins can manage users.")
        return redirect('dashboard')
...
def edit_user(request, user_id):
    ...
    if not request.user.groups.filter(name="Client Admin").exists():
        messages.error(request, "Access Denied: Only client admins can edit users.")
...
def toggle_user_status(request, user_id):
    ...
    if not request.user.groups.filter(name="Client Admin").exists():
        messages.error(request, "Access Denied: Only client admins can manage user statuses.")
        return redirect('manage-users')
        ...
        for record in records:
            ...
            if serializer.is_valid():
                try:
                    ...
                    qr.add_data(f"Receipt No: {sale.receipt_no}")
                    qr.make(fit=True)
                    ...
                    file_path = qr_code_dir / f"receipt_{sale.receipt_no}.png"

                    # Save QR code to file
                    with file_path.open("wb") as f:
                        img.save(f)

                    # Update the sale record with the QR code path
                    sale.qr_code = f"qr_codes/receipt_{sale.receipt_no}.png"
                    sale.save()
                    qr_codes.append(sale.qr_code)
